...
async def init_db():
    ...
    async with _pool.acquire() as conn:
        ...
        # Счётчики для профиля (продажи и успешные апгрейды — любой ранг,
        # кроме F, считается успешным)
        await conn.execute(
            "ALTER TABLE users ADD COLUMN IF NOT EXISTS phones_sold INTEGER NOT NULL DEFAULT 0"
        )
        await conn.execute(
            "ALTER TABLE users ADD COLUMN IF NOT EXISTS upgrades_successful "
            "INTEGER NOT NULL DEFAULT 0"
        )

        # Ежедневные бонусы: текущий день цикла (0 = ни разу не забирал) и
        # время последнего получения
        await conn.execute(
            "ALTER TABLE users ADD COLUMN IF NOT EXISTS daily_streak_day "
            "INTEGER NOT NULL DEFAULT 0"
        )
        await conn.execute(
            "ALTER TABLE users ADD COLUMN IF NOT EXISTS daily_last_claim_at "
            "DOUBLE PRECISION NOT NULL DEFAULT 0"
        )

        # Флаг "уже делал хотя бы один тел" — нужен для надёжного триггера
        # реферальной награды, не зависящего от общего числа телефонов
        # (которое может быть >1 ещё до первого «тел», например от промокода)
        await conn.execute(
            "ALTER TABLE users ADD COLUMN IF NOT EXISTS first_tel_done "
            "BOOLEAN NOT NULL DEFAULT FALSE"
        )
        ...
        await conn.execute(
            "ALTER TABLE users ADD COLUMN IF NOT EXISTS tester_probation "
            "BOOLEAN NOT NULL DEFAULT FALSE"
        )
...
async def increment_upgrades_successful(user_id: int):
    async with _pool.acquire() as conn:
        await conn.execute(
            "UPDATE users SET upgrades_successful = upgrades_successful + 1 WHERE user_id = $1",
            user_id,
        )
...
async def increment_phones_sold(user_id: int):
    async with _pool.acquire() as conn:
        await conn.execute(
            "UPDATE users SET phones_sold = phones_sold + 1 WHERE user_id = $1", user_id
        )
...
async def get_daily_status(user_id: int):
    """Возвращает (текущий_день, время_последнего_получения)."""
    async with _pool.acquire() as conn:
        row = await conn.fetchrow(
            "SELECT daily_streak_day, daily_last_claim_at FROM users WHERE user_id = $1",
            user_id,
        )
        if not row:
            return 0, 0
        return row["daily_streak_day"], row["daily_last_claim_at"]


async def set_daily_progress(user_id: int, day: int, claimed_at: float):
    async with _pool.acquire() as conn:
        await conn.execute(
            "UPDATE users SET daily_streak_day = $1, daily_last_claim_at = $2 WHERE user_id = $3",
            day, claimed_at, user_id,
        )


# ---------- ПЕРВЫЙ "ТЕЛ" (для реферальной награды) ----------

async def get_first_tel_done(user_id: int) -> bool:
    async with _pool.acquire() as conn:
        row = await conn.fetchrow(
            "SELECT first_tel_done FROM users WHERE user_id = $1", user_id
        )
        return bool(row["first_tel_done"]) if row else False


async def mark_first_tel_done(user_id: int):
    async with _pool.acquire() as conn:
        await conn.execute(
            "UPDATE users SET first_tel_done = TRUE WHERE user_id = $1", user_id
        )


# ---------- ТЕСТЕРЫ ----------

async def set_tester(user_id: int, probation: bool):
    async with _pool.acquire() as conn:
        await conn.execute(
            "UPDATE users SET is_tester = TRUE, tester_probation = $1 WHERE user_id = $2",
            probation, user_id,
        )


async def update_tester_probation(user_id: int, probation: bool) -> bool:
    """Возвращает False, если пользователь вообще не тестер."""
    async with _pool.acquire() as conn:
        row = await conn.fetchrow("SELECT is_tester FROM users WHERE user_id = $1", user_id)
        if not row or not row["is_tester"]:
            return False
        await conn.execute(
