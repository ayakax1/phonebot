...
async def cmd_add_phone(message: Message):
    ...
    if message.photo:
        ...
        photo_path = os.path.join(config.PHOTOS_DIR, f"{safe_name}.jpg")
        await bot.download_file(file.file_path, destination=photo_path)

    phone_id = db.add_phone(model, rarity, base_price, photo_path)
    await message.answer(
...
async def handle_tel_pull(message: Message):
    ...
    remaining = db.seconds_until_ready(user_id)
    if remaining > 0:
        await message.reply(
    ...
    db.set_last_pull_now(user_id)
    db.record_pull(user_id, phone["id"], status, price)
    ...
    if phone["photo_path"] and os.path.exists(phone["photo_path"]):
        await message.reply_photo(FSInputFile(phone["photo_path"]), caption=caption)
    else:
